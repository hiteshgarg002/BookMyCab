...
public class DisplayBookingByClient extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (rs.next()) {
				out.println(
						"<tr><th>S no.</th><th>Booking id</th><th>Client Email</th><th>Client Contact no</th><th>Vehicle Id</th><th>Booking Date</th><th>Booking</th><th>Source/Destination</th></tr>");
				int sn = 1;
				do {
					out.println("<tr align=center><td>" + sn + "</td><td>" + rs.getString("bookingid") + "</td><td>"
							+ rs.getString("clientemail") + "</td><td>" + rs.getString("mobile") + "</td><td>"
							+ rs.getString("vehicleid") + "</td><td>" + rs.getString("currentdate") + "</td><td>From "
							+ rs.getString("bfrom") + "<br> to " + rs.getString("bto") + "</td><td>"
							+ rs.getString("source") + " to " + rs.getString("destination") + "</td></tr>");
					sn++;
				} while (rs.next());
