...
public class VehicleController {
	...
	public static boolean AddNewRecord(AgencyVehicle V) {
		try {
			...
			String query = "insert into agencyvehicle(agencyid,vehicleid,vehiclename,modelyear,vehiclecolor,vehicletype,vehiclecapacity,insurance,ac,fueltype,picture) values('"
					+ V.getAgencyid() + "','" + V.getVehicleid() + "','" + V.getVehiclename() + "','" + V.getModelyear()
					+ "','" + V.getVehiclecolor() + "','" + V.getVehicletype() + "','" + V.getVehiclecapacity() + "','"
					+ V.getInsurance() + "','" + V.getAc() + "','" + V.getFueltype() + "','" + V.getVehiclepicture()
					+ "')";
	...
	public static AgencyVehicle DisplayById(String vid) {
		try {
			Connection cn = DBHelper.openConnection();
			String query = "select vehicleid,agencyid,vehiclename,modelyear,vehiclecolor,vehicletype,vehiclecapacity,insurance,ac,fueltype,picture from agencyvehicle where vehicleid='"
					+ vid + "'";
			ResultSet rs = DBHelper.executeQuery(query, cn);
			if (rs.next()) {
	...
	public static boolean EditRecord(AgencyVehicle V) {
		try {
			Connection cn = DBHelper.openConnection();
			String query = "update agencyvehicle set agencyid='" + V.getAgencyid() + "', vehiclename='"
					+ V.getVehiclename() + "', modelyear='" + V.getModelyear() + "', vehiclecolor='"
					+ V.getVehiclecolor() + "', vehicletype='" + V.getVehicletype() + "', vehiclecapacity='"
					+ V.getVehiclecapacity() + "', insurance='" + V.getInsurance() + "', ac='" + V.getAc()
					+ "', fueltype='" + V.getFueltype() + "'  where vehicleid='" + V.getVehicleid() + "'";
			System.out.println(query);
			boolean st = DBHelper.executeUpdate(query, cn);
			return (st);
	...
	public static boolean EditVehiclePicture(AgencyVehicle V) {
		try {
			Connection cn = DBHelper.openConnection();
			String query = "update agencyvehicle set picture='" + V.getVehiclepicture() + "' where vehicleid='"
					+ V.getVehicleid() + "'";
			System.out.println(query);
			boolean st = DBHelper.executeUpdate(query, cn);
			return (st);
	...
	public static ResultSet VehicleByCity(String city, String tp) {
		try {
			...
			String query = "";
			if (tpp <= 5) {
				query = "select v.* from agencyvehicle v ,agency a where v.agencyid=a.agencyid and a.city='" + city
						+ "' and v.vehiclecapacity between 1 and 5";
			} else {
				query = "select v.* from agencyvehicle v ,agency a where v.agencyid=a.agencyid and a.city='" + city
						+ "' and v.vehiclecapacity>5";
			}
			ResultSet rs = DBHelper.executeQuery(query, cn);
			return (rs);
		} catch (Exception e) {
