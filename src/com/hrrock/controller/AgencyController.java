...
public class AgencyController {
	...
	public static boolean AddNewRecord(Agency A) {
		try {
			...
			String query = "insert into agency(agencyname,ownername,address,state,city,contactperson,mobileno,phoneno,emailid,registrationid,registrationid_proof,identity,identity_proof,logo,password,status) values('"
					+ A.getAgencyname() + "','" + A.getOwnername() + "','" + A.getAddress() + "','" + A.getState()
					+ "','" + A.getCity() + "','" + A.getContactperson() + "','" + A.getMobileno() + "','"
					+ A.getPhoneno() + "','" + A.getEmailid() + "','" + A.getRegistrationid() + "','"
					+ A.getRegistrationid_proof() + "','" + A.getIdentity() + "','" + A.getIdentity_proof() + "','"
					+ A.getLogo() + "','" + A.getPassword() + "','" + A.getStatus() + "')";
	...
	public static Agency DisplayById(int aid) {
		try {
			Connection cn = DBHelper.openConnection();
			String query = "select a.agencyid,a.agencyname,a.ownername,a.address,a.state,(select s.statename from states s where s.stateid=a.state), a.city,(select c.cityname from cities c where c.cityid=a.city), a.contactperson,a.mobileno,a.phoneno,a.emailid,a.registrationid,a.registrationid_proof,a.identity,a.identity_proof,a.logo,a.password,a.status from agency a where a.agencyid="
					+ aid;
			ResultSet rs = DBHelper.executeQuery(query, cn);
			if (rs.next()) {
				Agency A = new Agency();
	...
	public static boolean EditRecord(Agency A) {
		try {
			Connection cn = DBHelper.openConnection();
			String query = "update agency set agencyname='" + A.getAgencyname() + "',ownername='" + A.getOwnername()
					+ "',address='" + A.getAddress() + "',state='" + A.getState() + "',city='" + A.getCity()
					+ "',contactperson='" + A.getContactperson() + "',mobileno='" + A.getMobileno() + "',phoneno='"
					+ A.getPhoneno() + "',emailid='" + A.getEmailid() + "',status='" + A.getStatus()
					+ "' where agencyid=" + A.getAgencyid();
			System.out.println(query);
			boolean st = DBHelper.executeUpdate(query, cn);
			return (st);
	...
	public static boolean EditRegistration(Agency A) {
		try {
			Connection cn = DBHelper.openConnection();
			String query = "update agency set registrationid='" + A.getRegistrationid() + "',registrationid_proof='"
					+ A.getRegistrationid_proof() + "' where agencyid=" + A.getAgencyid();
			System.out.println(query);
			boolean st = DBHelper.executeUpdate(query, cn);
			return (st);
	...
	public static boolean EditIdentity(Agency A) {
		try {
			Connection cn = DBHelper.openConnection();
			String query = "update agency set identity='" + A.getIdentity() + "', identity_proof='"
					+ A.getIdentity_proof() + "' where agencyid=" + A.getAgencyid();
			System.out.println(query);
			boolean st = DBHelper.executeUpdate(query, cn);
			return (st);
